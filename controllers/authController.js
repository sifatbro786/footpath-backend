...
export const register = async (req, res, next) => {
    try {
        ...
        console.log("📝 Registration attempt:", { name, email }); // Validation
        ...
        console.log("✅ User created successfully:", user._id); // Generate OTP for email verification

        const OTP = user.generateEmailVerificationOTP();
        await user.save({ validateBeforeSave: false });

        console.log("📧 Generated OTP:", OTP); // Send verification email

        try {
            await sendEmail({
                email: user.email,
                subject: `Email Verification OTP - ${process.env.SITE_NAME || "Footpath"}`,
            ...
            console.error("❌ Email sending failed completely:", emailError.message); // For development - auto verify if email fails
            if (process.env.NODE_ENV === "development") {
                console.log("🛠️ DEVELOPMENT: Auto-verifying email due to SMTP failure");
                user.isEmailVerified = true;
                user.emailVerificationOTP = undefined;
                user.emailVerificationOTPExpire = undefined;
                await user.save(); // Development-এ ডিফল্ট মেয়াদকাল সেট
...
export const verifyEmail = async (req, res, next) => {
    try {
        ...
        console.log("✅ Email verified successfully for:", user.email); // Send token response (auto login after verification)
...
export const resendVerification = async (req, res, next) => {
    try {
        ...
        console.log("📧 Resending OTP to:", email, "OTP:", OTP); // Send verification email

        try {
            await sendEmail({
                email: user.email,
                subject: `New Verification OTP - ${process.env.SITE_NAME || "Footpath"}`,
                template: "emailVerification",
                data: {
                    name: user.name,
                    otp: OTP,
...
export const login = async (req, res, next) => {
    try {
        ...
        console.log("✅ Login successful for user:", user._id); // 🚨 ফিক্স: টোকেন এক্সপায়ারির জন্য দিনের সংখ্যা সেট করুন
...
export const forgotPassword = async (req, res, next) => {
    try {
        ...
        // FIX (account enumeration): used to return 404 "No user found with this
        // email" here, letting an attacker check which emails are registered.
        // Always respond the same way; only actually send an OTP if the account exists.
        if (!user) {
        ...
        console.log("📧 Password reset OTP for:", email, "OTP:", OTP); // Send OTP email

        try {
            await sendEmail({
                email: user.email,
                subject: `Password Reset OTP - ${process.env.SITE_NAME || "Footpath"}`,
