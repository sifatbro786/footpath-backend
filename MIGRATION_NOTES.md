...
- **Before:** `heroRoutes.js` (`Hero`/`HeroItem` model) and `heroContentRoutes.js` (`HeroContent`
  model) were both mounted on `/api/v1/hero`. Since `heroContentRoutes` was mounted first,
...
- **Fix:**
  - `heroRoutes.js` now mounts on its own path: **`/api/v1/hero-items`** (was `/api/v1/hero`).
    `heroContentRoutes.js` keeps `/api/v1/hero`. **Frontend integration note:** any client
    call hitting `/api/v1/hero` for banner/slider items (not hero *content*) needs to move
    to `/api/v1/hero-items`.
...
2. **`orderController.js` → `createOrder`**: trusted `shippingPrice`, `taxPrice`,
   ...
   `POST /api/v1/orders` directly (skipping the checkout-preview call) with
...
```js
if (val_id && status === "VALID") { /* verify properly */ }
else { isVerified = true; }   // ⚠️ comment claimed "development mode" — NO env check existed
```
Anyone could send `GET /api/v1/payment/process-success?orderId=<any pending order id>` with
...
called here!` — and it was true. `POST/PUT/DELETE /api/v1/hero/admin/hero...` let anyone
...
- Added `POST /api/v1/admin/shipping/rates` (`createShippingRate`) so the admin panel can
...
|---|---|---|
| `routes/productRoutes.js` | `routes/admin/productAdminRoutes.js` | `/api/v1/admin/products` |
| `routes/couponRoutes.js` | `routes/admin/couponAdminRoutes.js` | `/api/v1/admin/coupons` |
| `routes/heroRoutes.js` (+ new `controllers/heroController.js`) | `routes/admin/heroAdminRoutes.js` (+ new `controllers/admin/heroAdminController.js`) | `/api/v1/admin/hero-items` |
| `routes/heroContentRoutes.js` | `routes/admin/heroContentAdminRoutes.js` | `/api/v1/admin/hero-content` |
| `routes/offerPopupRoutes.js` | `routes/admin/offerPopupAdminRoutes.js` | `/api/v1/admin/offers` |
| `routes/productCampaignRoutes.js` | `routes/admin/productCampaignAdminRoutes.js` | `/api/v1/admin/product-campaigns` |
| `routes/promotionRoutes.js` | `routes/admin/promotionAdminRoutes.js` | `/api/v1/admin/promotions` |
| `routes/reviewRoutes.js` | `routes/admin/reviewAdminRoutes.js` | `/api/v1/admin/reviews` |
| `routes/categoryRoutes.js` | `routes/admin/categoryAdminRoutes.js` | `/api/v1/admin/categories` |
| `routes/navbarRoutes.js` | `routes/admin/navbarAdminRoutes.js` | `/api/v1/admin/navbar` |
| `routes/aplusContentRoutes.js` | `routes/admin/aplusContentAdminRoutes.js` | `/api/v1/admin/aplus-content` |
| `routes/pageMetaRoutes.js` | `routes/admin/pageMetaAdminRoutes.js` | `/api/v1/admin/page-meta` |
| *(was `routes/uploadRoutes.js`, now removed — fully admin)* | `routes/admin/uploadAdminRoutes.js` | `/api/v1/admin/upload` |
...
that the mount path itself says `/admin` (e.g. `POST /api/v1/aplus-content/admin/dashboard`
→ `GET /api/v1/admin/aplus-content/dashboard`). Full before/after list: see the table above
...
3. **`uploadRoutes.js`** — all three upload endpoints (`/single`, `/multiple`, `/offer`)
   ...
   spam risk). Moved fully under `/api/v1/admin/upload`.
4. **`productRoutes.js`** — `GET /admin/dashboard` (the *non-optimized* variant) had no auth
